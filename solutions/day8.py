...
encoding_gains = {"letters" : 0, "backslash" : 2, 
        "quote" : 2, "hex" : 1}
...
def count_encoding(string, pattern):
    ...
    if matched is not None:
        out += sum(encoding_gains[group] * len(captures) for group, captures in matched.capturesdict().items())
    return out

result = [parse_string(string, pattern) for string in raw_input]
